...
public:
  ...
  //! Unassigned block id.
  static constexpr uint32_t kUnassignedId = 0xFFFFFFFFu;
...
public:
  ...
  Error add(RAWorkReg* workReg, RATiedFlags flags, RegMask allocable, uint32_t useId, uint32_t useRewriteMask, uint32_t outId, uint32_t outRewriteMask, uint32_t rmSize = 0) noexcept {
  ...
  Error addCallRet(RAWorkReg* workReg, uint32_t outId) noexcept {
    ...
    RegGroup group = workReg->group();
    RegMask allocable = Support::bitMask(outId);
...
public:
  ...
  static constexpr uint32_t kCallArgWeight = 80;
  ...
  //! Logger flags.
  FormatFlags _formatFlags = FormatFlags::kNone;
  ...
  inline RAInst* newRAInst(RABlock* block, RATiedFlags flags, uint32_t tiedRegCount, const RARegMask& clobberedRegs) noexcept {
  ...
  inline Error assignRAInst(BaseNode* node, RABlock* block, RAInstBuilder& ib) noexcept {
