...
public:
  ...
  FormatFlags _formatFlags = FormatFlags::kPositions;
  StringTmp<512> _sb;
#endif
      ...
      if (node->isInst()) {
        ...
        if (cf != InstControlFlow::kRegular) {
          ...
          if (cf == InstControlFlow::kJump || cf == InstControlFlow::kBranch) {
            ...
            if (!Support::test(inst->instOptions(), InstOptions::kUnfollow)) {
  ...
  void _logNode(BaseNode* node, uint32_t indentation, const char* action) noexcept {
    ...
    Formatter::formatNode(_sb, _formatFlags, cc(), node);
    _sb.append('\n');
    _logger->log(_sb);
  }
#else
