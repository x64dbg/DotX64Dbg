  ...
  for (RegGroup group : RegGroupVirtValues{}) {
    ...
    for (i = 0; i < count; i++) {
      ...
      if (tiedReg->hasUseId()) {
        ...
        RegMask allocableRegs = tiedReg->allocableRegs();
    ...
    if (usePending) {
      ...
      for (i = 0; i < count; i++) {
        ...
        if (!tiedReg->hasUseId()) {
          RegMask allocableRegs = tiedReg->allocableRegs() & ~(willFree | willUse);
    ...
    for (i = 0; i < dupTiedCount; i++) {
      ...
      Support::BitWordIterator<RegMask> it(tiedReg->_allocableRegs);
      while (it.hasNext()) {
    ...
    if (outPending) {
      // Live registers, we need a separate variable (outside of `_curAssignment) to hold these because of KILLed
      ...
      for (i = 0; i < outTiedCount; i++) {
        ...
        if (physId == RAAssignment::kPhysNone) {
          RegMask allocableRegs = tiedReg->_allocableRegs & ~(outRegs | avoidRegs);
...
Error RALocalAllocator::allocBranch(InstNode* node, RABlock* target, RABlock* cont) noexcept {
  ...
  if (target->hasEntryAssignment()) {
    ...
    if (curCursor != injectionPoint) {
      ...
      node->clearInstOptions(InstOptions::kShortForm);
