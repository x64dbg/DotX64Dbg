...
static inline JitAllocatorPrivateImpl* JitAllocatorImpl_new(const JitAllocator::CreateParams* params) noexcept {
  ...
  size_t poolCount = 1;
  if (uint32_t(options & JitAllocatorOptions::kUseMultiplePools) != 0)
...
static JitAllocatorBlock* JitAllocatorImpl_newBlock(JitAllocatorPrivateImpl* impl, JitAllocatorPool* pool, size_t blockSize) noexcept {
  ...
  if (bitWords != nullptr) {
    if (uint32_t(impl->options & JitAllocatorOptions::kUseDualMapping) != 0) {
  ...
  if (uint32_t(impl->options & JitAllocatorOptions::kFillUnusedMemory) != 0) {
...
Error JitAllocator::release(void* rxPtr) noexcept {
  ...
  if (block->areaUsed() == 0) {
    if (pool->emptyBlockCount || uint32_t(impl->options & JitAllocatorOptions::kImmediateRelease) != 0) {
