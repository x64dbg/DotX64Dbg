...
public:
  inline FuncSignatureT(CallConvId ccId = CallConvId::kHost, uint32_t vaIndex = kNoVarArgs) noexcept {
    static const TypeId ret_args[] = { (TypeUtils::TypeIdOfT<RET_ARGS>::kTypeId)... };
...
public:
  ...
  template<typename T>
  inline void setRetT() noexcept { setRet(TypeUtils::TypeIdOfT<T>::kTypeId); }
  ...
  template<typename T>
  inline void setArgT(uint32_t index) noexcept { setArg(index, TypeUtils::TypeIdOfT<T>::kTypeId); }
  ...
  template<typename T>
  inline void addArgT() noexcept { addArg(TypeUtils::TypeIdOfT<T>::kTypeId); }
...
//! Attributes are designed in a way that all are initially false, and user or FuncFrame finalizer adds them when
//! necessary.
enum class FuncAttributes : uint32_t {
  ...
  kX86Avx512Enabled = 0x00020000u,

  //! This flag instructs epilog writer to emit EMMS instruction before RET (X86).
  kX86MmxCleanup = 0x00040000u,

  //! This flag instructs epilog writer to emit VZEROUPPER instruction before RET (X86).
  kX86AvxCleanup = 0x00080000u,

  //! Function has aligned save/restore of vector registers.
  kAlignedVecSR = 0x40000000u,
  //! FuncFrame is finalized and can be used by PEI.
  kIsFinalized = 0x80000000u
};
ASMJIT_DEFINE_ENUM_FLAGS(FuncAttributes)

//! Function detail - CallConv and expanded FuncSignature.
//!
//! Function detail is architecture and OS dependent representation of a function. It contains calling convention
//! and expanded function signature so all arguments have assigned either register type & id or stack address.
class FuncDetail {
public:
  //! Calling convention.
  CallConv _callConv;
...
public:
  ...
  inline bool isAvxEnabled() const noexcept { return hasAttribute(FuncAttributes::kX86AvxEnabled); }
  //! Enables AVX use.
  inline void setAvxEnabled() noexcept { addAttributes(FuncAttributes::kX86AvxEnabled); }
  //! Disables AVX use.
  inline void resetAvxEnabled() noexcept { clearAttributes(FuncAttributes::kX86AvxEnabled); }

  //! Tests whether the function has AVX-512 enabled.
  inline bool isAvx512Enabled() const noexcept { return hasAttribute(FuncAttributes::kX86Avx512Enabled); }
  //! Enables AVX-512 use.
  inline void setAvx512Enabled() noexcept { addAttributes(FuncAttributes::kX86Avx512Enabled); }
  //! Disables AVX-512 use.
  inline void resetAvx512Enabled() noexcept { clearAttributes(FuncAttributes::kX86Avx512Enabled); }

  //! Tests whether the function has MMX cleanup - 'emms' instruction in epilog.
  inline bool hasMmxCleanup() const noexcept { return hasAttribute(FuncAttributes::kX86MmxCleanup); }
  //! Enables MMX cleanup.
  inline void setMmxCleanup() noexcept { addAttributes(FuncAttributes::kX86MmxCleanup); }
  //! Disables MMX cleanup.
  inline void resetMmxCleanup() noexcept { clearAttributes(FuncAttributes::kX86MmxCleanup); }

  //! Tests whether the function has AVX cleanup - 'vzeroupper' instruction in epilog.
  inline bool hasAvxCleanup() const noexcept { return hasAttribute(FuncAttributes::kX86AvxCleanup); }
  //! Enables AVX cleanup.
  inline void setAvxCleanup() noexcept { addAttributes(FuncAttributes::kX86AvxCleanup); }
  //! Disables AVX cleanup.
  inline void resetAvxCleanup() noexcept { clearAttributes(FuncAttributes::kX86AvxCleanup); }
