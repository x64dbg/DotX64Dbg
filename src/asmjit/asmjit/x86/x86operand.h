...
public:
  ...
  template<RegType REG_TYPE>
  static inline RegGroup groupOfT() noexcept { return RegTraits<REG_TYPE>::kGroup; }

  template<RegType REG_TYPE>
  static inline TypeId typeIdOfT() noexcept { return RegTraits<REG_TYPE>::kTypeId; }
  ...
  static inline OperandSignature signatureOfVecByType(TypeId typeId) noexcept {
    return OperandSignature(
      typeId <= TypeId::_kVec128End ? RegTraits<RegType::kX86_Xmm>::kSignature :
      typeId <= TypeId::_kVec256End ? RegTraits<RegType::kX86_Ymm>::kSignature : RegTraits<RegType::kX86_Zmm>::kSignature);
  }

  static inline OperandSignature signatureOfVecBySize(uint32_t size) noexcept {
    return OperandSignature(
      size <= 16 ? RegTraits<RegType::kX86_Xmm>::kSignature :
      size <= 32 ? RegTraits<RegType::kX86_Ymm>::kSignature : RegTraits<RegType::kX86_Zmm>::kSignature);
  }
