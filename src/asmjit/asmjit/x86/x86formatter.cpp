  ...
  if (Operand::isVirtId(id)) {
    if (emitter && emitter->emitterType() == EmitterType::kCompiler) {
      ...
      if (cc->isVirtIdValid(id)) {
        ...
        if (vReg->type() != type && uint32_t(type) <= uint32_t(RegType::kMaxValue) && uint32_t(formatFlags & FormatFlags::kRegCasts) != 0) {
...
ASMJIT_FAVOR_SIZE Error FormatterInternal::formatOperand(
  ...
  if (op.isMem()) {
    ...
    if (off || !m.hasBaseOrIndex()) {
      ...
      uint32_t base = 10;
      if (uint32_t(formatFlags & FormatFlags::kHexOffsets) != 0 && off > 9) {
  ...
  if (op.isImm()) {
    ...
    if (uint32_t(formatFlags & FormatFlags::kHexImms) != 0 && uint64_t(val) > 9) {
      ASMJIT_PROPAGATE(sb.append("0x", 2));
