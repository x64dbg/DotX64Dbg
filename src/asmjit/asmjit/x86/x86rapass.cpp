...
static ASMJIT_FORCE_INLINE RATiedFlags raMemBaseRwFlags(OpRWFlags flags) noexcept {
  ...
  return (RATiedFlags)raUseOutFlagsFromRWFlags(OpRWFlags(uint32_t(flags) >> shift) & OpRWFlags::kRW);
}
  ...
  if (Inst::isDefinedId(instId)) {
      ...
      for (uint32_t i = 0; i < opCount; i++) {
        ...
        if (op.isReg()) {
          ...
          if (vIndex < Operand::kVirtIdCount) {
            ...
            RegMask allocable = _pass->_availableRegs[group] & allowedRegs;
          ...
          if (mem.hasIndexReg()) {
            ...
            if (vIndex < Operand::kVirtIdCount) {
              ...
              RegGroup group = workReg->group();
              RegMask allocable = _pass->_availableRegs[group] & instructionAllowedRegs;
              ...
              ASMJIT_PROPAGATE(ib.add(workReg, RATiedFlags::kUse | RATiedFlags::kRead, allocable, useId, useRewriteMask, outId, outRewriteMask));
            }
          }
        }
      }
    ...
    if (inst->hasExtraReg()) {
      ...
      if (vIndex < Operand::kVirtIdCount) {
        ...
        if (group == RegGroup::kX86_K) {
          ...
          RegMask allocableRegs = _pass->_availableRegs[group];
          ASMJIT_PROPAGATE(ib.add(workReg, RATiedFlags::kUse | RATiedFlags::kRead, allocableRegs, BaseReg::kIdBad, rewriteMask, BaseReg::kIdBad, 0));
          singleRegOps = 0;
        }
        else {
          // REP {cx|ecx|rcx} register - read & write, allocable to {cx|ecx|rcx} only.
          ASMJIT_PROPAGATE(ib.add(workReg, RATiedFlags::kUse | RATiedFlags::kRW, 0, Gp::kIdCx, rewriteMask, Gp::kIdBad, 0));
        }
      }
      else {
    ...
    if (hasGpbHiConstraint) {
      for (RATiedReg& tiedReg : ib) {
        tiedReg._allocableRegs &= tiedReg.hasFlag(RATiedFlags::kX86_Gpb) ? 0x0Fu : 0xFFu;
      }
    }
  ...
  for (uint32_t i = 0; i < opCount; i++) {
    ...
    if (op.isReg()) {
      ...
      if (vIndex < Operand::kVirtIdCount) {
        ...
        RegGroup group = workReg->group();
        RegMask allocable = _pass->_availableRegs[group];
        ASMJIT_PROPAGATE(ib.add(workReg, RATiedFlags::kUse | RATiedFlags::kRead, allocable, ret.regId(), 0, BaseReg::kIdBad, 0));
      }
    }
    else {
      return DebugUtils::errored(kErrorInvalidAssignment);
...
Error X86RAPass::emitMove(uint32_t workId, uint32_t dstPhysId, uint32_t srcPhysId) noexcept {
  ...
  if (uint32_t(_formatFlags & FormatFlags::kAnnotations) != 0) {
